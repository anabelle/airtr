...
import { calculateBookValue } from './fleet.js';
import { fp, fpToNumber, fpScale } from './fixed-point.js';
...
describe('fleet', () => {
    describe('calculateBookValue', () => {
        ...
        it('applies depreciation over 1 year', () => {
            ...
            const expectedValue = fpToNumber(fpScale(mockModel.price, Math.pow(0.9, 1)));
        ...
        it('applies depreciation over 10 years', () => {
            ...
            const expectedValue = fpToNumber(fpScale(mockModel.price, Math.pow(0.9, 10)));
            expect(fpToNumber(value)).toBeCloseTo(expectedValue, 0);
        });
        ...
        it('applies condition penalty for worn aircraft', () => {
            ...
            const expectedBase = fpScale(mockModel.price, Math.pow(0.9, 5));
        ...
        it('applies high utilization penalty', () => {
            ...
            const baseWithoutPenalty = fpScale(mockModel.price, Math.pow(0.9, 5));
        ...
        it('does not penalize normal utilization', () => {
            ...
            const value = calculateBookValue(mockModel, normalHours, 1.0, manufactureTick, fiveYearsLater);
            const expectedBase = fpScale(mockModel.price, Math.pow(0.9, 5));
            expect(fpToNumber(value)).toBeCloseTo(fpToNumber(expectedBase), 0);
        });
        ...
        it('combines all factors correctly', () => {
            ...
            const depValue = fpScale(mockModel.price, Math.pow(0.9, 10));
