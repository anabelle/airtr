...
const WORLD_BANK_GDP_URL = 'https://api.worldbank.org/v2/en/indicator/NY.GDP.PCAP.CD?downloadformat=csv';
...
const BUSINESS_IATAS = new Set([
    'ATL', 'PEK', 'LHR', 'HND', 'DXB', 'ORD', 'DFW', 'JFK', 'CDG', 'AMS',
    'FRA', 'IST', 'MAD', 'BCN', 'MUC', 'ZRH', 'VIE', 'FCO', 'CPH', 'ARN',
    'HEL', 'DUB', 'MAN', 'LGW', 'BRU', 'LIS', 'OSL', 'DME', 'LED', 'DOH',
    'AUH', 'RUH', 'JED', 'SIN', 'HKG', 'ICN', 'PVG', 'CAN', 'TPE', 'BKK',
    'KUL', 'CGK', 'BOM', 'DEL', 'SYD', 'MEL', 'LAX', 'SFO', 'SEA', 'DEN',
    'PHX', 'LAS', 'MSP', 'DTW', 'MIA', 'MCO', 'IAH', 'EWR', 'BOS', 'DCA',
    'IAD', 'YYZ', 'YVR', 'YUL', 'MEX', 'GRU', 'GIG', 'SCL', 'LIM', 'BOG',
    'EZE', 'CPT', 'JNB', 'CAI', 'NBO', 'ADD'
]);

const SKI_IATAS = new Set([
    'GVA', 'INN', 'SZG', 'ZRH', 'MUC', 'GNB', 'LYS', 'TRN', 'MXP', 'VCE',
    'SLC', 'DEN', 'ASE', 'EGE', 'JAC', 'YYC', 'YVR', 'YXS', 'UIO', 'SCL'
]);

const BEACH_IATAS = new Set([
    'CUN', 'PUJ', 'NAS', 'MBJ', 'CZM', 'AUA', 'SJU', 'HNL', 'OGG', 'LIR',
    'DPS', 'SYD', 'OOL', 'BNE', 'MIA', 'FLL'
]);
...
const downloadFile = (url, outputPath, maxRedirects = 10) => new Promise((resolve, reject) => {
    if (maxRedirects <= 0) {
...
const parseWorldBankGdp = (zipPath) => {
    ...
    const entry = zip.getEntries().find((e) =>
        e.entryName.startsWith('API_NY.GDP.PCAP.CD') && !e.entryName.startsWith('Metadata')
    );
    if (!entry) {
        throw new Error('World Bank GDP CSV not found in zip');
    }
    const content = entry.getData().toString('utf8').replace(/^\uFEFF/, '').replace(/\r/g, '');
...
const classifyTags = (airport, population) => {
    ...
    if (BEACH_IATAS.has(airport.iata) || (Math.abs(airport.latitude) <= 30 && airport.altitude < 500)) {
        tags.push('beach');
        return tags;
    }
...
const toIso3 = (iso2) => {
    const map = {
        ...
        NP: 'NPL', NL: 'NLD', NZ: 'NZL', NI: 'NIC', NE: 'NER', NG: 'NGA', KP: 'PRK', MK: 'MKD',
        NO: 'NOR', OM: 'OMN', PK: 'PAK', PW: 'PLW', PA: 'PAN', PG: 'PNG', PY: 'PRY', PE: 'PER',
        PH: 'PHL', PL: 'POL', PT: 'PRT', QA: 'QAT', RO: 'ROU', RU: 'RUS', RW: 'RWA', KN: 'KNA',
        LC: 'LCA', VC: 'VCT', WS: 'WSM', SM: 'SMR', ST: 'STP', SA: 'SAU', SN: 'SEN', RS: 'SRB',
        SC: 'SYC', SL: 'SLE', SG: 'SGP', SK: 'SVK', SI: 'SVN', SB: 'SLB', SO: 'SOM', ZA: 'ZAF',
        KR: 'KOR', SS: 'SSD', ES: 'ESP', LK: 'LKA', SD: 'SDN', SR: 'SUR', SE: 'SWE', CH: 'CHE',
...
async function main() {
    ...
    for (const airport of rawAirports) {
        ...
        const population = iso2 ? findNearestCityPopulation({ ...airport, countryCode: iso2 }, cityIndex) : null;

        const airportPopulation = population ?? 250000;
        const airportGdp = gdp ?? 15000;

        const tags = classifyTags({ ...airport, iata: airport.iata, altitude: airport.altitude, latitude: airport.latitude }, airportPopulation);
