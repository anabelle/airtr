...
describe("buildActionDTag", () => {
  ...
  it("adds entity id, tick, and seq for route actions when seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:dev-v3:action:route_open:rt-123:100:s7");
  });
  ...
  it("adds entity id, tick, and seq for aircraft actions when seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:dev-v3:action:aircraft_purchase:ac-777:555:s42");
  });
  ...
  it("returns base tag with only seq when no identifiers are present but seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:dev-v3:action:hub_add:s3");
  });
  ...
  it("ensures different seq values produce different d-tags for same action", () => {
    ...
    expect(tag1).toBe("airtr:world:dev-v3:action:route_update_fares:rt-abc:200:s0");
    expect(tag2).toBe("airtr:world:dev-v3:action:route_update_fares:rt-abc:200:s1");
  });
  ...
  it("omits seq suffix when seq is undefined", () => {
    ...
    expect(withoutSeq).toBe("airtr:world:dev-v3:action:route_assign_aircraft:rt-1:50");
  });
});
