...
describe("replayActionLog", () => {
  ...
  it("applies a lease buyout once when replaying a leased aircraft checkpoint", async () => {
  ...
  it("does not charge a lease buyout twice when replaying against an already-owned optimistic checkpoint", async () => {
    const pubkey = "pubkey-buyout-optimistic";
    const model = getAircraftById("atr72-600");
    expect(model).toBeTruthy();
